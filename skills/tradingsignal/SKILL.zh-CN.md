...
**信号图排在最前面。** `get_indicators` 会返回一张：当前真正表态的指标直接画在价格图上，每类最多两个、总共最多六个。它用一张图回答"哪些读数看多、哪些看空、每一个在哪里这么说"，所以它排在所有方法图之上——方法图画的是某一个方法的内部结构，这张画的是结论。它会跟着读数收窄：用 `categories` 或 `side` 过滤，图上就只画同一批。它画的是最后一根**已收盘** K 线并带自己的 `barTs`；如果和读数的 `barTs` 不同，说明最新一根还在走，要说清你指的是哪一根。
...
1. **信号**——趋势强度、共振数（几个看多几个看空），以及推动它的那几个表态读数，配信号图。这一条排第一，因为它就是读者真正会看的那张图。
