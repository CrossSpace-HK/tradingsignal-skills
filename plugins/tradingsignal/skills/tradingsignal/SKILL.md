...
- **DeMark across timeframes is one call: `get_td9_read`.** It reads the symbol's own set together: 5m/15m/1h/4h/1d where the symbol serves intraday bars, and 1d/1w/1mo for a daily-only name (for example futures, indices, HK/KS listings) — never expect intraday rows there. One conclusion, the 9s and 13s behind it, each timeframe's stage. Answer from `structured`; `text` is a four-line summary, and its `createdAt`/`ageSeconds` date the paragraph only. The data's age is `freshness[timeframe]` (`dataAsOf`, `fetchedAt`, `stale`): name a stale timeframe rather than using it silently. One timeframe's raw counts, TDST and chart: `get_method_analysis(engine="td9")`.
