...
- **回答里要放图？先读[附图的规矩](references/chart-links.zh-CN.md)**——链接写法、图链接 vs 方法页、`conclusion`、预设、哪张图配哪条 bullet。要附图时这一步不是可选的。
...
- **趋势强弱的数字取自 `trendStrength`，不要从图上读。** 它就是 `trendChart` 所画的数：`score`/`direction`/`threshold`；五个分项 `sections`（趋势、动量、量能、波动、形态——各自带 `strength`、`direction` 和 `indicators`，即实际参与计算的指标数）；`series` 是滚动强弱，最多最近 120 根已收盘 K 线，旧的在前，每根带 `regime` 字段（不要用这个标签写结论，用大白话说）；`seriesBars` 说明实际有几根。要说清是哪个分项在撑这个分数。字段含义见[单标的分析](references/symbol-analysis.zh-CN.md)。
...
1. **信号**——趋势强弱（分数、方向、有没有超过无趋势阈值、哪个分项在撑）、共振数，以及按 `notability` 排序的表态读数。两张头图放这里。
